...
export class TelegramBotService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    this.bot.hears('View watchlist', async (ctx) => {
      ...
      await ctx.reply(
        ...
        Markup.inlineKeyboard([
          ...watchlist.map((m) => [
            Markup.button.callback(m.title, `show_${m.id}`),
          ]),
        ]),
      );
    });
    ...
    this.bot.on('text', async (ctx) => {
      ...
      const movieData = await this.moviesSearchService.searchByTitle(title, year);
      if (!movieData) return ctx.reply('Movie not found.');
